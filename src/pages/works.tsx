  ...
  return (
    <>
      ...
      <div className="w-full max-w-6xl m-auto py-20 px-8">
        <h1 className="font-bold text-3xl mb-7 text-gable tracking-wide">Our Work</h1>
        <div className="grid grid-cols-12">
          {expandedWork && <div className="col-span-12 text-center relative">
            <img className="w-full" src={_props.data[expandedWork.id].edges[workSlide].node.childImageSharp.fluid.src} />
            <div className="absolute top-0 right-0 left-0 bottom-0 bg-black block bg-opacity-70">
              <div className="absolute top-1/2 left-1/2 transform -translate-y-2/4 -translate-x-2/4 w-4/5">
                <h2 className="text-pelorous font-bold text-3xl">{expandedWork.title}</h2>
                <h3 className="text-pelorous font-light text-2xl mb-20">{expandedWork.year}</h3>
                <p className="text-white text-justify text-xl font-light" dangerouslySetInnerHTML={{__html: expandedWork.fullDescriptiion}}/>
              </div>
              <button 
                className="absolute top-14 right-14 opacity-50 hover:opacity-100 focus:outline-none transition-all"
                onClick={() => compressWork()}
              >
                <StaticImage src="../images/close.svg" alt="Close" placeholder="none" width={22} />
              </button>
              <button 
              ...
              >
                <StaticImage src="../images/left-arrow-light.svg" alt="Left" placeholder="none" width={40} />
              </button>
              <button 
              ...
              >
                <StaticImage src="../images/right-arrow-light.svg" alt="Left" placeholder="none" width={40} />
              </button>
            </div>
          </div>}
          {!expandedWork && worksContent.map((work:Work) => {
            return <>
              <div className="col-span-4 relative">
                <img src={_props.data[work.id].edges[0].node.childImageSharp.fluid.src} alt={work.title}/>
                <div className="absolute top-0 right-0 left-0 bottom-0 bg-black opacity-0 block transition-all bg-opacity-50 hover:opacity-100">
                  <div className="absolute top-1/2 left-1/2 transform w-full text-center -translate-y-2/4 -translate-x-2/4">
                    <h2 className="text-pelorous font-bold text-2xl mb-2">{work.title}</h2>
